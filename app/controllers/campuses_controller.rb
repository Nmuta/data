class CampusesController < ApplicationController
  ...
  def get_campuses
    campuses = Campus.all.map{|c| {"id" =>c.id, "name"=>c.name} }
    the_host = Rails.env=="production" ? "https://"+request.host : "http://localhost:3000"
    respond_to do |format|
      format.json { render json: {campuses: campuses} }
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @campus.save
        format.html { redirect_to @campus, notice: 'Campu was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @campus.update(campus_params)
        format.html { redirect_to @campus, notice: 'Campu was successfully updated.' }
